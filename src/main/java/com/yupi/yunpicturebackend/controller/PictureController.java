...
import com.yupi.yunpicturebackend.api.imagesearch.ImageSearchApiFacade;
...
import com.yupi.yunpicturebackend.model.vo.PictureTagCategory;
...
public class PictureController {
    ...
    private final Cache<String, String> LOCAL_CACHE = Caffeine.newBuilder()
            ...
            .expireAfterWrite(Duration.ofMinutes(5))  // 缓存 5 分钟移除
                    .build();
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPageWithCache(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        if (cachedValue != null) {
            //如果redis缓存命中，更新本地缓存，缓存结果
            LOCAL_CACHE.put(cacheKey, cachedValue);
            Page<PictureVO> cachedPage = JSONUtil.toBean(cachedValue, Page.class);
            return ResultUtils.success(cachedPage);
        }
        // 3. 本地缓存和redis缓存都没有命中，查询数据库
        Page<Picture> picturePage = pictureService.page(new Page<>(current, size),
                pictureService.getQueryWrapper(pictureQueryRequest));
    ...
    public BaseResponse<PictureTagCategory> listPictureTagCategory() {
        ...
        List<String> tagList = Arrays.asList("热门", "写实主义", "理想主义", "表现主义", "抽象主义", "装饰主义", "观念艺术", "超现实主义", "极简主义");
        List<String> categoryList = Arrays.asList("油画", "雕塑", "国画", "摄影", "建筑艺术", "动漫");
